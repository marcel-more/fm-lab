...
<!-- components:auto --> *Components: schema 1.32.0 · xml-import 5.5.0 (converter 2.30.0) · fm-spec 2.9.0 · plugin-spec 1.2.0*

**FileMaker 26 compatibility across the whole stack.** The XML import reads FileMaker 26 exports (SaXML 2.3.0.0) without loss under an explicit profile, the FileMaker reference and the script generator cover the FileMaker 26 language at the shape and at the documentation level, and the web client shows the new FileMaker 26 field metadata. Alongside: fm-spec extended with script-trigger compatibility, error codes, feature versions, together with the tests and views that read them.

- **XML import — FileMaker 26 exports read without loss** — every file is imported under an explicit **SaXML profile** read from the export's root version (`saxml22` for FileMaker 19–22, `saxml23` for FileMaker 26 and later; persisted in `FilesCatalog`, named in the import report), and extractions that exist in only one form run only for that profile. Existing catalogs rebuild on the next import
  - **What FileMaker 26 moves or adds** — value-list options embedded in the value-list node were silently lost and now fill `OptionsForValueLists` in both forms; field **annotations** and **customized display names** (`FieldsForTables.Field_Annotation`, `FieldDisplayNames`), **table-view columns** with their `Layout → Field` links (`LayoutTableViewColumns`) and the `comment` of a base table (`BaseTableCatalog.BT_Comment`) are extracted for the first time
  - **Disabled definitions no longer count** — FileMaker 26 exports the disabled auto-enter, lookup and validation definitions that FileMaker 22 omits (`enable="False"`); they produced phantom calculations, where-used links and variable usages. They are now flagged (`*_Enabled`, `CalculationsCatalog.Is_Enabled`), stay visible, and produce no usage links
  - **Field entry behaviour** — the per-mode entry restriction of a field on a layout (browse / find: allowed, selection only, display only, governed by a formula) is extracted (`LayoutObjects.Entry_Browse` / `Entry_Find`, calculation role `field_entry`). FileMaker files its formula under the hide-condition key, so it used to surface as a "Hide Condition" without text that had taken over the entry formula's references; anchor and edges now carry their own slot, and the layout-quality rules cover the entry formula next to the hide condition
  - **New link roles, and a merge fix** — the FileMaker 26 script steps carry their field roles (`Perform Find by Natural Language`, `Perform RAG Action`, the PDF family), `Export Field Contents` is now `exports_from_field`, and `Re-Login` links its external data source. A table that dropped to zero rows in a re-export no longer keeps its old rows

- **Built-in functions — one node per function, whatever language the formula was written in** — FileMaker's export carries a built-in only as a name in the UI language of the client that wrote the formula, so a localized spelling became its own node and a single `Get ( PageNumber )` fell apart into up to three objects with three different where-used answers. Every token is now resolved against the standard reference and registered under its **canonical English name** (`BuiltinFunctionIdentity`: reference ID, canonical name, namespace); tokens the reference does not know (dynamic `Get ( $var )`, keyword constants, functions newer than the bundled reference) keep their name-based identity, and the mobile functions that appeared as synthetic plug-in calls in localized exports are re-typed as well. A valid `{{Symbol}}` in a text object is the display-time value of `Get ( Symbol )` and now carries a `displays_symbol` edge to that node, so one page answers "who shows this" and "who computes with it"

- **The FileMaker reference — the FileMaker 26 language, and three new Claris tables** (fm-spec 2.9.0)
  - **Two shape coverages, FileMaker 22 and 26** — the XML grammar of every script step is verified separately against a FileMaker 22.0.6 and a FileMaker 26.0.2 corpus of the same coverage solution: the ten FileMaker 26 steps carry their options, the FileMaker 26 changes to existing steps live as overrides next to the 22 shape, and 205 of 206 FileMaker 22 shapes are instance-paired. `GET /api/reference/steps/:id/grammar?coverage=22|26` resolves a step for either coverage with the same rule the generator uses and tags every row as standard or override
  - **Documentation coverage FileMaker 26** — names, categories, compatibility, one-liners, option prose, signatures and parameters in all 11 locales follow the current Claris help; changed compatibility is marked (`since FM 26:`), and `functions.removed_in_version` names the functions a newer FileMaker no longer resolves (`SetPersistentData`, `FindPersistentData`)
  - **Script-trigger compatibility** — every trigger event carries the same seven-platform matrix as the script steps (`trigger_compat`, tri-state: *Partial* stays Partial, never "undocumented")
  - **Error codes** — the FileMaker error-code table as data (`error_codes`, ranges such as `5000-5499` included, web-only codes marked, messages in 11 locales), resolvable via `GET /api/reference/error-codes/:code`
  - **Feature versions, named constants, numeric version keys** — the "features not compatible with previous versions" page as `feature_versions`, the full Claris list of named constants as `language_constants` (52 rows, each with the functions it is a parameter value of — the only safe way to tell the constant `Lower` from the function `Lower`), and a numeric version key on steps, functions and features, so a version is never compared as text, where `8.5` would sort above `19.0`
  - **What the export leaves out** — the script-step slots the SaXML export never carries are constraint rows scoped per FileMaker version (`saxml_omission`: the XSLT stylesheet of an XML export, the *automatically open* / *create e-mail* flags, the zoom formula of `Set Zoom Level`, the `Option` flag of `Insert Embedding`) instead of a prose list; the FileMaker 26 spellings the decompiler translates for a FileMaker 22 target are reference data too (`coverage_renames`)

- **Script generator** (`fm-generate-script`)
  - **Coverage-aware generation** — emission, decompile, lint and gate follow the selected target (`--coverage 22|26`), so a draft is written against the FileMaker version it is pasted into
  - **The reverse direction keeps every state FileMaker writes** — measured on the coverage corpus, the decompile → generate round trip now reproduces the clipboard form of 1,623 of 1,636 FileMaker 22 instances (before: 1,230) and 1,719 of 1,749 FileMaker 26 instances (before: 1,316); every remaining difference is a named residue. Localized clipboard copies are canonicalized (`Hole ( Kontoname )` → `Get ( AccountName )`, never inside string literals or comments), states that used to vanish on the way to text render explicitly, and the decompiler re-parses its own rendering — where the positional form would bind a value to the wrong option it writes the labeled form and says so. The reference gained the shapes that were missing along the way (the XML-source form of `Import Records`, the base-table comment of `Truncate Table`, the hulls FileMaker persists unfilled) and binding forms that state per option which element or attribute FileMaker drops
  - **Save Records as PDF writes what FileMaker writes** — a paste probe in FileMaker 22.0.6 and 26.0.2 settled the long-open questions about this step, as reference data rather than generator code: the document title FileMaker stores twice is declared as a mirror (`step_mirror_elements`) and written once in a draft, the `appearance` attribute FileMaker discards on paste is marked as dropped (`step_options.paste_dropped`), and a step without PDF options emits exactly the option tree FileMaker itself writes
  - **Rules from the reference instead of constants** — the value form of a target slot (`step_options.slot_kind`) rejects a field in a variable-only slot at every stage (lint L010 through gate G307), where FileMaker itself would silently paste the field name as a variable name; option couplings FileMaker drops without their counterpart are reference rows (L009), Open/Commit/Revert Transaction carry pairing constraints, boolean XML domains are per attribute, and a comparison of `Get ( LastError )` against a code the error table does not list is reported (L011)

- **Web client**
  - **Detail views for built-in functions, plug-in functions and plug-in components** — identity and reference metadata (category, return type, origin version, component size), links to the Claris help page or the vendor doc-set with the vendor's online page as fallback, and a usage summary per link role and object type; the component view lists its member functions with deduplicated totals
  - **Field detail — extended options (FileMaker 26)** — annotation, customized display names as a table of elements (key → label or formula) with the formula itself, and disabled auto-enter / lookup / validation definitions marked as such
  - **Schema viewer** — new *Triggers* and *Error Codes* tabs (a number in the filter finds the code or its range, text searches the message), a coverage switch on the grammar block with override rows badged, "SaXML gap" badges with their version scope, and grammar blocks grouped into the value sets FileMaker actually applies instead of seemingly contradictory lines, with display labels next to the keys
  - **Docs ↔ fm-spec cross-navigation** — with the Claris help doc-set installed, every step and function page links to its Claris help page and the help page back, keyed by the internal ID rather than the localized name; the Settings page shows both coverages (FileMaker version of the shapes and of the docs)

- **Analysis tests & inventory queries**
  - **New test "Version Floor"** — every FileMaker file declares a minimum version of its own, and that declaration does not follow the features added later. The test derives the version a file actually needs from the introduction versions of the script steps, built-in functions and script triggers it contains, and reports per file — highest requirement first — the features that push the floor above the declaration, each with its usage count and an example object to jump to. The summary also answers the neutral question ("which FileMaker version does this scope need at all?"), so the test is useful before a rollout, a client upgrade or a server move, not only when something is wrong. Disabled steps count, and a feature Claris publishes no version for is never counted as old
  - **New test "Layout Merge Anchors"** — finds the anchors that render literally at runtime: `{{symbols}}` the standard reference does not know as `Get` parameters and `<<Field>>` anchors without field resolution. Neither is an error to FileMaker — the raw text simply stays in the layout, visible to the user and silent to every log. A layout symbol inventory ships alongside, grouped by the canonical `Get` parameter (`{{Seitennummer}}` and `{{PageNumber}}` count as one symbol)
  - **Platform and error-code checks read the new reference data** — the WebDirect and Go tests now cover the triggers attached to layouts, layout objects and the file, plus a member for the events that exist only in FileMaker Go; the script error checks report the literals a script compares with `Get ( LastError )` — unknown codes as a warning, web-only codes as info, and a neutral inventory of every compared code
  - **Documentation inventories** — four custom queries read a solution's schema-level documentation in one pass, each row opening the table or field detail view: table comments, field comments, and — FileMaker 26 only — field annotations and customized display names (a formula label is prefixed with "ƒ")

- **Testing & cleanup**
  - **fm-lab's own coverage solution replaces external test data** — every script step and built-in function of FileMaker 22 and 26, one layout per object type, data model, value lists, custom functions, triggers and security, plus a companion file for external references, exported from **both FileMaker versions** with and without DDR info. The conversion test imports them one SaXML profile per run into throwaway databases and checks every row, link and calculation count against a frozen expectation table (more than 1,200 values across four sets)
  - **SaXML shape diff (quality test T8)** — compares the form of every script step and layout object (parameter sequence, boolean option sequence, element and attribute sets) between two exports of the same solution — typically FileMaker 22 and 26 — and fails on every delta outside the baseline, so a new attribute or element of a SaXML version change can no longer pass unnoticed
  - **External reference installers retired** — `install-ooe-fm` and `install-fm-xml-export-exploder` are no longer part of the delivery; the bundled coverage solution replaces them as the converter's test corpus, and their doc-set and skill pages are gone. 
  - **Docs update** — CLAUDE.md, the schema reference (61 link roles), the XML-schema and pipeline references are updated for the SaXML profiles and the new tables
  - **Fix** — the Leiden clustering hung forever on an edgeless graph (a micro-solution whose only operational links are built-ins or local variables) and kept the import lock; it now returns the trivial partition
