...
--
-- KANONISCHE QUELLE: DIESE Datei — die LogicalLinks-Definition lebt genau
-- einmal, hier in P5. rest-api/templates/sql/graph_logical_links.sql ist nur
-- noch ein Konsumenten-Stub (SELECT auf die View, read-only-Vorschau im
-- Template-Pool) und trägt keine eigene Definition mehr — es gibt KEINE
-- Sync-Pflicht mehr. Konsumenten der View: Graph-Endpoints
-- (graph_subgraph/graph_overview_*/graph_depth_profile via READ_ONLY-Kopie),
-- tools/graph-export (ClusterEdges-Kette). Änderungen hier sind
-- Schnittstellen-Änderungen für diese Konsumenten.
--
-- VERSIONS-HISTORIE der View-Definition (vormals im API-Template gepflegt):
--   v1.1.0  Promotion in P5 (vorher 3× inline dupliziert: Export, Subgraph,
--           Skill-Adhoc — drifteten auseinander)
--   v1.2.0  Stufe C: lokale Variablen ($x) ausgeschlossen (s. Block unten)
--   v1.3.0  Graph-Policy: trigger_script + OnWindowTransaction-Feld-Kandidaten
--           raus (Trigger-Knoten waren reine Script-Satelliten)
--   v1.4.0  Feld-Verankerung: Objekt-Trigger-Spiegel aufs angezeigte Feld
--           umgelenkt (Existenz-Semantik; button_action bleibt Layout)
--   v1.5.0  Spezialtypen Chart/Web Viewer nicht mehr gehoisted (s. Block unten)
