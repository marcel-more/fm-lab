/* GENERIERT von ingestion/gen_streamify_sql.sh aus ingestion/sql/convert_xml_01_extract.sql
   NICHT von Hand editieren — Änderungen in der Basis bzw. ingestion/sql/streamify/ vornehmen.
...
privilege_sets AS (
    SELECT
        unnest(xml_extract_elements('<ObjectList>' || ObjectList || '</ObjectList>', '/ObjectList/PrivilegeSet')) as ps_xml
    FROM read_xml(getvariable('fm_xml'), record_element='PrivilegeSetsCatalog', maximum_file_size=getvariable('dom_threshold'), streaming=getvariable('use_streaming'), columns={'ObjectList':'VARCHAR'})
    WHERE ObjectList IS NOT NULL
)
