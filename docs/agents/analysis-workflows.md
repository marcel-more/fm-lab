...
- **DDR-Info is optional.** `DDR_ScriptSteps`/`DDR_Calculations` are only populated when `XMLMetadata.Has_DDR_INFO = 'True'` — check before relying on formula-chunk analysis; degrade gracefully (see the CASE pattern in `query-cookbook.md`). A file with `FilesCatalog.Has_DDR_INFO = false` imports without error and looks complete (objects, `CalculationsCatalog.Formula_Text`), but carries **no formula references at all** — no `reads_field`/`sets_field` from calculations, no `calls_function`/`calls_customfunction`/`calls_pluginfunction`. Two sibling export defects hide plug-in calls the same way: `<Function Missing>` in `Formula_Text` (plug-in not loaded on the exporting client — the name is gone from the export) and a dynamic function name `MBS($name; …)` (no object, no edge; evidence only in `PluginFunctionUsages` × `MBS_SubnameMap`). Before concluding "this solution uses no plug-in" or "this field is unused", run the `plugin-availability` test (`/fm-test`, rubric `metadata-integrity`) — its census names the affected files and the cause.
