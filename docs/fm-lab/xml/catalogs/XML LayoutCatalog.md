...
- **Version difference:** SaXML v2.3.0.0 (FileMaker 26) adds the **table-view columns** of every layout (`TableView/ObjectList/TableViewLayoutObject` with `hidden`, `id`, `name`, `width` and a `FieldReference` per column) and the formula of a field's `by_calculation` **entry mode** (`<CanEntryCalc>`; v2.2.x writes the state bits only). Two existing spellings change: `DisplayCalculations@membercount` is always 12 — FileMaker 26 pads the list, and the surplus anchors repeat the first slot's hash or carry foreign chunks — and `Part@type` of a `Part@kind="5"` reads `Trailing Sub-summary` instead of the Claris mislabel `Trailing Grand Summary`.
